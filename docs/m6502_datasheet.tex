...
\begin{titlepage}
    ...
    {\large GPIO, Timer, UART, and SK6812 LED Controller\\[2cm]}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Integrated Peripherals}:
        \begin{itemize}
            ...
            \item SK6812: Hardware RGB LED controller with pin multiplexing
            \item Clock Control: Runtime CPU clock division
        \end{itemize}
    \item \textbf{Pin Multiplexing}: UART, SK6812, and GPIO can share physical pins via mode registers
...
\begin{enumerate}
    ...
    \item \textbf{Memory-Mapped Peripherals}: GPIO, UART, Timer, SK6812 LED, Clock Control
\end{enumerate}
...
0xA010 -- 0xA017 & 8 bytes & SK6812 LED controller \\
...
\begin{itemize}
    ...
    \item SK6812 DATA mode (0x03) -- LED controller output
\end{itemize}
...
0x03 & SK6812\_DATA -- LED controller output \\
0x04-0xFF & Reserved \\
\bottomrule
\end{tabular}
...
\subsection{SK6812 LED Controller (0xA010-0xA017)}

Hardware controller for SK6812/WS2812 addressable RGB LEDs with precise timing.
...
\midrule
\endhead
0xA010 & CONTROL & Control register (write 1 to start) \\
0xA011 & CLKDIV & Clock divider for bit timing \\
0xA012 & RED & Red component (0-255) \\
0xA013 & GREEN & Green component (0-255) \\
0xA014 & BLUE & Blue component (0-255) \\
0xA015 & WHITE & White component (0-255, unused for RGB) \\
0xA016 & STATUS & Status register (bit 0 = BUSY) \\
0xA017 & Reserved & -- \\
\bottomrule
\caption{SK6812 Register Map}
\end{longtable}

\subsubsection{Pin Configuration}

The SK6812 data output must be routed to a GPIO pin using the mode register:

\begin{lstlisting}[language=asm6502, caption=Configure SK6812 output on GPIO4]
LDA #$03          ; SK6812_DATA mode
STA $A008         ; MODE_PIN4
\end{lstlisting}

\subsubsection{Protocol Sequence}

To update an LED:
\begin{enumerate}
    \item Wait for BUSY=0 (read STATUS register, bit 0)
    \item Write RED, GREEN, BLUE values (WHITE is ignored for RGB LEDs)
    \item Write 0x01 to CONTROL to start transmission
    \item Repeat for each LED in chain
    \item After last LED, wait 50$\\mu$s+ for latch
\end{enumerate}

\subsubsection{Example: Updating LED Strip}

\begin{lstlisting}[language=asm6502, caption=Update 3-LED RGB strip]
; Configure GPIO4 as LED data output
LDA #$03
STA $A008         ; MODE_PIN4 = SK6812_DATA

; LED 0: Red
wait_0:
    LDA $A016     ; Read STATUS
    AND #$01      ; Check BUSY
    BNE wait_0    ; Wait if busy
...
; Wait for latch (50$\\mu$s+ idle time)
; Implement delay loop here
\end{lstlisting}
...
SK6812 Bit Period & Configurable & Set via CLKDIV register \\
\bottomrule
\end{tabular}
...
MCU Integration & 216 \\
\midrule
\textbf{Total RTL} & \textbf{~2,900 lines} \\
\bottomrule
\end{tabular}
